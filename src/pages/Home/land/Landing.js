...
const Landing = () => {
    ...
    useEffect(() => {
        ...
        TweenMax.from(text.current, 0.9, { x: -300, opacity: 0, ease: Power3.easeInOut });
        TweenMax.to(text.current, 0.9, { x: 0, opacity: 1, ease: Power3.easeInOut });
        
        tl.to(imgContainer.current, 0.9, { clipPath: 'polygon(25% 0%, 100% 0%, 75% 100%, 0% 100%)', ease: Power2.easeInOut })
          .from(img.current, 0.9, { scale: 1.7, ease: Power2.easeInOut, delay: -1.9 });
    }, []);
